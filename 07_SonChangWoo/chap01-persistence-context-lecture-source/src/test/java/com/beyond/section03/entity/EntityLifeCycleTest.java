package com.beyond.section03.entity;
...
class EntityLifeCycleTest {
  ...
  void testTransient(int menuCode) {
    ...
    assertTrue(entityManager.contains(foundMenu)); // entityManager가 foundMenu를 포함하면 성공
    // newMenu는 영속성 컨텍스트에서 관리 되지 않는 비영속 상태의 객체
    assertFalse(entityManager.contains(newMenu)); // entityManager가 newMenu를 포함하지 않으면 성공
  ...
  void testManagedOtherEntityManager(int menuCode) {
    ...
    // 새로운 엔티티가 생섬됨 -> 서로 독립된 영속성 컨텍스트를 관리
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    // (현재)준영속 상태 -> DB 테이블과 매핑 X, 그냥 자바 객체의 값만 변경됨
    //    (더티 체킹 X)
  ...
  void testMergeSave() {
    ...
    entityManager.merge(foundMenu); // 병합
    // 1차 캐시와 준영속 객체를 병합하여 영속 객체를 반환
    // -> 더티 채킹 -> menu_code(PK)가 999인 행이 없음 확인함
    // -> INSERT 구문 생성
